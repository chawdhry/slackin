...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey }){
  let div = dom('.splash',
    ...
    dom('p',
      'Join Our secret most active', dom('b', name),
    ...
    dom('p.status',
        ...
        ? [
          dom('b.active', 1092), ' users online now of ',
          dom('b.total', 11283), ' registered.'
        ]
        : [dom('b.total', 11283), ' users are registered so far.']
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
    ...
    !iframe && dom('footer',
      'proudly run by ',
      dom('a href=http://rauchg.com/slackin target=_blank', 'Indian cryptocurrency maniacs')
    ),
