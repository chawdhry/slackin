...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey }){
  let div = dom('.splash',
    ...
    dom('p',
      'Join Our Undergrounded  ', dom('b', name),
    ...
    dom('p.status',
      active
        ? [
          dom('font color=red', 598), ' users online now of ',
          dom('b', 11283), ' registered.'
        ]
        :[dom('font color=red', 766), ' users online now of ', dom('b', 11283), ' users are registered so far.']
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
      ...
      dom('input.form-item type=email name=email placeholder=enter_your_Email_here_to_get_invitation '
        + (!iframe ? 'autofocus' : '')),
      dom('br'),
      ...
      dom('button.loading', 'send me the invitation to my E-mail ID')
    ),
                dom('br'),
                dom('p','NOTE: Invitation link sent to your email is valid for next 10 minutes only. Check your e-mail inbox as quickly as possible'),
    dom('p.signin',
      'or ',
    ...
    !iframe && dom('footer',
      'proudly run by ',
      dom('a href=#', 'Indian cryptocurrency maniacs')
    ),
